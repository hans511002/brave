...
//#endif
		protected:
 			inline void _init(TimeZone tz=localTimeZone)
			{
				...
				//int l=(GetTickCount() % this->MillisPerSecond )*this->TicksPerMillisecond;		//精确到毫秒
					...
					case StaUTC:				// Mon May 14 02:03:55 2007	// 星期几 月份 日期 时:分:秒 年
						{
							...
							if(len!=7)EXP(String("输入字符时间\"")+String(s)+String("\"不是形如格式的\"Mon May 14 02:03:55 2007\"字符串"));
							for(int i=1;i<=12;i++)
							{
