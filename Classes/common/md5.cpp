...
namespace Common
{
	...
	void md5::MD5Init (MD5_CTX * context)       
	{
		...
		context->state[0] = 0x67452301;// 给出4个32比特的初始向量.                                    
	...
	//
	//MD5 分组更新操作.不断地对消息块进行MD5 消息-摘要操作 继续一个MD5操作,处理另一个消息分组并更新context.                                     
	// 输入分组 输入的分组的长度  context                                     
	//
	...
	// MD5 最终结果. 以一个 MD5 报文摘要操作结束, 写下报文摘要值 报文摘要   context                                     
	...
	string  md5::ConvertChar(unsigned char digest[16],int type)
	{
		unsigned int i;
		if(type==16)
		{
	...
	{  
		FILE *fp;
		if (!(fp=fopen(filename.c_str(),"rb"))) //二进制打开
		{
		...
		MD5Init(&context);  
		while (len = (int)fread (buffer, 1, 16, fp))  
