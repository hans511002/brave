...
namespace engine{
	...
	std::map<std::string, tinyxml2::XMLDocument *> XMLConfigParser::xmlCache;

	tinyxml2::XMLDocument * XMLConfigParser::getDoc(std::string filePath, std::string name){
		std::map<std::string, tinyxml2::XMLDocument *>::iterator it = xmlCache.find(name);
		if (it != xmlCache.end()){
			return  it->second;
		}
		else{
	...
	bool XMLConfigParser::removeDoc(std::string fileName, std::string name){
		std::map<std::string, tinyxml2::XMLDocument *>::iterator it = xmlCache.find(name);
		if (it != xmlCache.end()){
			tinyxml2::XMLDocument* doc = it->second;
			xmlCache.erase(it);
			doc->~XMLDocument();
			return true;
		}
	...
	tinyxml2::XMLElement * XMLConfigParser::findChild(tinyxml2::XMLElement * root, std::string childName){
		tinyxml2::XMLElement *cnode = root->FirstChildElement();
		while (cnode){
	...
	tinyxml2::XMLElement *  XMLConfigParser::findChild(tinyxml2::XMLElement * root, std::string nodePath,  std::string &attrName){
		...
		tinyxml2::XMLElement * cnode = root; 
		while (i < n  && root){
	...
	vector<tinyxml2::XMLElement *> XMLConfigParser::findChildList(tinyxml2::XMLElement * root, std::string nodePath){
		nodePath = Common::String::Trim(nodePath, '.'); 
	...
	vector<tinyxml2::XMLElement *> XMLConfigParser::getChildList(tinyxml2::XMLElement * root, std::string nodePath){
		return findChildList(root, nodePath);
	};
	vector<tinyxml2::XMLElement *> XMLConfigParser::getChildList(std::string nodePath){
		//std::vector<std::string> elems;
		//split(nodePath, '.', elems);
		//tinyxml2::XMLElement * node;
		//int n = elems.size()-1;
		//size_t i = 0;
		tinyxml2::XMLElement * cnode = doc->RootElement();
		if (!cnode){
			return vector<tinyxml2::XMLElement *>();
		}
		//while (i < n  && cnode){
		//	std::string & elName = elems.at(i);
		//	i++;
