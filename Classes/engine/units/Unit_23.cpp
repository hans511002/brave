...
namespace engine 
{
	...
	{ 
		...
		{
			typeUnit = 23;
			container = new Unit_mc(this, "Unit23_mc", typeUnit); //new Unit23_mc(); 
			...
			if(world->listOfFirePortals.size() > 0)
			{
				...
				this->portalAnima = new MovieClip("unit/", "RoadUnitFire_mc", "RoadUnitFire_mc");// RoadUnitFire_mc();
		...
		void Unit_23::kill()
		{
			if(!dead && health <= 0)
			{
				...
				if(direction != "up")
				{
					tempObject = new MovieClip(this->world, "unit/", "Unit23HorisontDeath_mc", "Unit23HorisontDeath_mc");// new Indexes(new Unit23HorisontDeath_mc(), 1);
				}
				else
				{
					tempObject = new MovieClip(world, "unit/", "Unit23VerticalDeath_mc", "Unit23VerticalDeath_mc");//Indexes(new Unit23VerticalDeath_mc(), 1);
				}
