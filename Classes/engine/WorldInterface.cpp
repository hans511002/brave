...
namespace engine {
    WorldInterface::WorldInterface () : container (NULL), fasterFlag (0), world (NULL), castMask (NULL), lastTime (NULL), sellHint (NULL), getAllHint (NULL),
                                        fireCount (0), iceCount (0), stoneCount (0), levinCount (0), getAllCount (0), fireBuyHistory (0), iceBuyHistory (0),
                                        stoneBuyHistory (0), levinBuyHistory (0), blockCastGolem (false), blockCastIceman (false), blockCastAir (false),
                                        castGolemCounter (0), castIcemanCounter (0), castAirCounter (0), castGolemTimer (0), castIcemanTimer (0),
                                        castAirTimer (0) {
    ...
    bool WorldInterface::init ()// (event:Event) : void
    {
        ...
        this->container->fireSphereMyPoint = CONVERT_TO_WORLD_POS(
                this->container->fireSphere->localToGlobal (this->container->fireSphereSphereCase->getPosition ()));
        this->container->iceSphereMyPoint = CONVERT_TO_WORLD_POS(
                this->container->iceSphere->localToGlobal (this->container->iceSphereSphereCase->getPosition ()));
        this->container->stoneSphereMyPoint = CONVERT_TO_WORLD_POS(
                this->container->stoneSphere->localToGlobal (this->container->stoneSphereSphereCase->getPosition ()));
        this->container->levinSphereMyPoint = CONVERT_TO_WORLD_POS(
                this->container->levinSphere->localToGlobal (this->container->levinSphereSphereCase->getPosition ()));
    ...
    void WorldInterface::update (float dt) {
        if (this->mouseEnabled) {
            ...
            if (!this->container->getAllOpened) {
                if (this->container->fireSphere->isVisible () && this->container->iceSphere->isVisible () && this->container->stoneSphere->isVisible () &&
                    this->container->levinSphere->isVisible ()) {
                            ...
                            if (this->world->towerMenu) {
                                ...
                                if (this->world->ultraTowerMenu->myTower->spheresManage ("scan") > 0) {
                                    if (this->world->ultraTowerMenu->myTower->towerType == 5) {
                                        if (this->world->getSphere->fireCount > 0 && this->world->getSphere->iceCount == 0 &&
                                            this->world->getSphere->stoneCount == 0 && this->world->getSphere->levinCount == 0) {
                                            if (!this->world->ultraTowerMenu->myTower->shootingTurnStack[0]) {
                                                this->world->getSphere->getSphereFromArchiveToTower ();
                                            }
                                        } else if (this->world->getSphere->fireCount == 0 && this->world->getSphere->iceCount == 0 &&
                                                   this->world->getSphere->stoneCount > 0 && this->world->getSphere->levinCount == 0) {
    ...
    void WorldInterface::updateInfo () {
        ...
        std::setText (this->container->buyGetAllBuyTXT, Main::mainClass->readXMLClass.listOfFirePriceXML[this->fireBuyHistory]
                                                        + Main::mainClass->readXMLClass.listOfIcePriceXML[this->iceBuyHistory] +
                                                        Main::mainClass->readXMLClass.listOfStonePriceXML[this->stoneBuyHistory]
        ...
        if (param1 == "buy") {
            if (param2 == "fire") {
                ...
                if (this->world->money >= Main::mainClass->readXMLClass.listOfFirePriceXML[this->fireBuyHistory] +
                                          ...
                                          Main::mainClass->readXMLClass.listOfLevinPriceXML[this->levinBuyHistory]) {
                    this->getAllCount++;
                    int tempObject = Main::mainClass->readXMLClass.listOfFirePriceXML[this->fireBuyHistory] +
                                     Main::mainClass->readXMLClass.listOfIcePriceXML[this->iceBuyHistory] +
                                     Main::mainClass->readXMLClass.listOfStonePriceXML[this->stoneBuyHistory] +
                                     Main::mainClass->readXMLClass.listOfLevinPriceXML[this->levinBuyHistory];
                    if (Main::moneyDebug)
    ...
    void WorldInterface::spheresBlockManage (string param1) {
        if (param1 == "") {
            ...
            this->container->buyGetAll->setVisible (true);
